...
# it uses an env_var FLANNEL_REVDEPS to manage this that gets whatever params are passed to the modules
# this way we can load a module twice provided we are giving it different options
_flannel_track_revdeps() {
  # we didn't eat this
  if [[ "$FLANNEL_REVDEPS" != *":${1}"* ]]; then 
    export FLANNEL_REVDEPS="${FLANNEL_REVDEPS}:${1}";
    return 1
  fi
}